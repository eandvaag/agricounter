...
def predict(sch_ctx, image_set_dir, request):
...
def train_baseline(sch_ctx, root_dir):
    ...
    log = json_io.load_json(os.path.join(root_dir, "log.json"))
    ...
    while True:
        ...
        if sch_ctx["prediction_queue"].size() > 0:
            return False
